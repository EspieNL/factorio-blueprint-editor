...
import FD, { getOrderedQualities } from '../core/factorioData'
...
export class InventoryDialog extends Dialog {
    ...
    private getQualityOptions(): Array<{ id: string; label: string }> {
        return getOrderedQualities().map(quality => ({
            id: quality.name,
            label: quality.localised_name || quality.name,
        }))
    }
    ...
    private updateQualitySelector(): void {
        ...
        const selectedQuality =
            this.getQualityOptions().find(quality => quality.id === this.m_SelectedQuality) ||
            this.getQualityOptions()[0]
        this.m_QualityLabel.text = selectedQuality?.label || 'Normal'
    }
}
